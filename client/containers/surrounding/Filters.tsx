...
export default function Filters({
  translations: { sortText, sortData, limitData, limitText, filtersText },
}: FiltersProps) {
  ...
  const filters = useMemo(
    ...
    [sortText, sortData, limitData, limitText],
  );
  ...
  function deleteFilterHander(paramData: string) {
    searchParams.set('page', '1');

    searchParams.delete(paramData.split('=')[0] as 'sort' | 'limit');
  ...
  return (
    <section className="flex flex-col ~/md:~my-12/24">
              ...
              return (
                <li key={paramData}>
                  <div className="flex items-center space-x-1 rounded-full border border-gray-300 px-4 py-1 text-sm dark:border-gray-600">
                    ...
                    <FaXmark
                      onClick={() => deleteFilterHander(paramData)}
